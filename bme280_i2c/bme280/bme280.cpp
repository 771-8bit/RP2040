/*
AdafruitのBME280のArduinoライブラリをRaspberry Pi Pico C/C++ SDK用に改変
I2C専用
...
void BME280::write8(uint8_t reg, uint8_t value)
{
    ...
    i2c_write_blocking(i2c_default, _i2caddr, buffer, 2, false);
}
...
uint8_t BME280::read8(uint8_t reg)
{
    ...
    i2c_write_blocking(i2c_default, _i2caddr, buffer, 1, true); // true to keep master control of bus
    i2c_read_blocking(i2c_default, _i2caddr, buffer, 1, false); // false - finished with bus
...
uint16_t BME280::read16(uint8_t reg)
{
    ...
    i2c_write_blocking(i2c_default, _i2caddr, buffer, 1, true); // true to keep master control of bus
    i2c_read_blocking(i2c_default, _i2caddr, buffer, 2, false); // false - finished with bus
...
uint32_t BME280::read24(uint8_t reg)
{
    ...
    i2c_write_blocking(i2c_default, _i2caddr, buffer, 1, true); // true to keep master control of bus
    i2c_read_blocking(i2c_default, _i2caddr, buffer, 3, false); // false - finished with bus
...
bool BME280::takeForcedMeasurement(void)
{
    ...
    if (_measReg.mode == MODE_FORCED)
    {
        ...
        while (read8(BME280_REGISTER_STATUS) & 0x08)
        {
            ...
            if ((to_ms_since_boot(get_absolute_time()) - timeout_start) > 2000)
            {
                return_value = false;
                break;
            }
